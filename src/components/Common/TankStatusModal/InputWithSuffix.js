...
class InputWithSuffix extends Component {
  ...
  handleEditOnClick = () => {
    this.setState({ border: true, onActive: true }, () =>
  ...
  handleHiddenBorder = () => {
    this.setState({ border: false, onActive: false })
  }
  ...
  render() {
    const { border, onActive } = this.state
    const { classes, inputType, value, isEdit } = this.props
    if (inputType == "baseInput") {
      return (
        <div
          className={`${classes.root} tank_status`}
          style={{
            border: `1px solid ${onActive ? "#00A19C" : "#D9D9D9"}`,
            background:`${onActive ? "#F0F9F9" : "none"}`
          }}
        >
          {!onActive ? (
            <span
              style={{ display: "inline-block", width: "100%", paddingLeft: 5 }}
            ...
            <InputBase
              onBlur={() => this.setState({ onActive: false })}
