...
const SalesTab = ({ data, onChange, inventoryData }) => {
  ...
  const onVarianceControlChange = (value, field) => {
    ...
    newInventoryData[YESTERDAY_SALES_FINAL_FIGURE] =
      Number(data?.actual_sales) + Number(newData?.yesterday_sales_adjustment)
    newData[SALES_FINAL_FIGURE] =
      Number(data?.actual_sales) + Number(newData?.yesterday_sales_adjustment)
    newData[SALES_VARIANCE] =
      Number(data?.actual_sales) +
      Number(newData?.yesterday_sales_adjustment) -
    ...
    <div>
      <div className="row">
        <div className="form-group col-md-6">
          <label>
            ACTUAL SALES (L)<span className="extra-lbl">D-1</span>
          </label>
          <input
        ...
        <div className="form-group col-md-6">
          <label>
            EXPECTED SALES (L)<span className="extra-lbl">D-1</span>
          </label>
          <input
            className="form-control awsm-input"
