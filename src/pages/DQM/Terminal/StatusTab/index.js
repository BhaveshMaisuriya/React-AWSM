...
const TabStatus = props => {
  ...
  const [isInactiveDateRangeTouched, setIsInactiveDateRangeTouched] =
    useState(false)
  ...
  useEffect(() => {
    if (data?.status_awsm === 'Temp Inactive') {
      if (
        !data?.inactive_date_range_1?.type ||
  ...
  const onchangeHandler = (field, value) => {
    ...
    if (fl.length >= 2) {
      ...
      if (
        ...
        newData['inactive_date_range_1'] = {
          ...previousInactiveDateRange.current,
        }
      }
    }
    setData(newData)
    ...
    <div className="dqm-status-container">
      <div className="row">
        ...
        <div className="col-12 col-sm-6">
          <label>
            INACTIVE DATE RANGE
            {data?.status_awsm === 'Temp Inactive' && !scheduler && (
