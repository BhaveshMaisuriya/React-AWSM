...
import BryntumChartTable, {
  bryntumSchedulerTableNameForCookie,
} from './OrderBankTable/BryntumChartTable'
...
function OrderBank({
  ...
  const [showSendMultiNotification, setShowSendMultiNotification] =
    useState(false)
  ...
  const [isCustomizeGanttModalOpen, setIsCustomizeGanttModalOpen] =
    useState(false)
  ...
  const [bryntumCurrentColumns, setBryntumCurrentColumns] = useState(() => {
    if (!getCookieByKey(bryntumSchedulerTableNameForCookie))
      return ganttChartTableDefaultColumns
    const cookieParseData = JSON.parse(
      getCookieByKey(bryntumSchedulerTableNameForCookie)
    )
  ...
  const reloadUploadDMR = async () => {
    ...
    await getRTSOrderBankTableData({
      ...
      search_fields: '*',
      q:
        payloadFilter?.filterQuery !== null ||
  ...
  const isShiftDateCorrect = date => {
    const diff = moment
      .duration(moment().startOf('day').diff(moment(date, 'YYYY-MM-DD')))
      .asDays()
  ...
  useEffect(() => {
    if (multipleorder) {
      ...
      setDeleteMultipleStatus(
        multipleorder.order_banks !== undefined ? 'success' : 'error'
      )
  ...
  const onSendOrderBankDN = () => {
    ...
    selectAll
      ? sendMultipleOrderBankDN({ filter: filterOrderBank, all: true })
      : sendMultipleOrderBankDN({
          order_id: orderBankTableData.filter(e => e.isChecked).map(e => e.id),
        })
  }
  ...
  useEffect(() => {
    ...
    orderBankTableData !== null &&
      orderBankTableData.map((item, index) => {
    ...
    if (sendDN) {
      let checkDN = checkedData.filter(
        v => (v.dn_no === null || v.dn_no === '') && v.dn_status !== 'Pending'
      )
      sendDN.disabled =
        checkDN?.length > 0 && checkedData.length === checkDN?.length
          ? false
          : true
    }
  ...
  const onChangeRegionHandler = region => {
    setRegionTable(region)
    setTerminalTable(
      REGION_TERMINAL.find(item => item.region === region).terminal[0]
    )
    resetTableAndFilters()
  }

  const onChangeTerminalHandler = terminal => {
    setTerminalTable(terminal)
    resetTableAndFilters()
  }

  const onChangeSchedulingStatus = status => {
    setStatusDropdown(status == 'All' ? '' : status)
    resetTableAndFilters()
  }
                ...
                <Row className="remove_border">
                  <Col lg={9} className="order-bank-bar">
                    ...
                    <div className="order-bank-region">
                      <AWSMDropdown
                        value={regionTable}
                        onChange={value => onChangeRegionHandler(value)}
                        items={REGION_TERMINAL.map(e => e.region)}
                      />
                    </div>
                    <div className="order-bank-region ml-2">
                    ...
                    <div className="order-bank-region">
                      <AWSMDropdown
                        ...
                        onChange={value => {
                          onChangeSchedulingStatus(value)
                        }}
                  ...
                  <Col
                    lg={3}
                    className="order-bank-bar right"
                    id="order-bank-main"
                  >
                    ...
                    <span className="m-0 order-bank-label">
                      <OrderBankSummary
                        data={orderBankTableSummary}
                        totalOrders={totalRow}
                      />
                    </span>
                  </Col>
                </Row>
...
const mapDispatchToProps = dispatch => ({
  ...
  onGetDeleteMultipleOrder: payload =>
    dispatch(getdeleteMultipleOrder(payload)),
